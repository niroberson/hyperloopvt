...
if(strcmp(parameters,'Second-2D'))
    ...
    rho_alum = 2.8*1e-8; % Resistivity of track (Ohm*m)
...
if(strcmp(parameters,'Third-2D'))
    ...
    Br = 1.277; % Magnet remanence (T)
    h = 0.05; % Heigh of permanent magnet (m)
    rho_alum = 3.94*1e-8; % Resistivity of track (Ohm*m)
...
if(strcmp(parameters,'Fourth-2D'))
    % Magnet Parameters
    M = 4; % Number of Magnets in Wavelength
    tau = 0.1; % Pole pitch (m)
    p = pi/tau; 
...
if(strcmp(parameters,'Magplane'))
    ...
    rho_alum = 1.68*1e-8; % Resistivity of track (Ohm*m)
...
if(strcmp(geometry,'Single'))
    Bz = @(y,z) Bsz(y,z) - (-R2*C3.*exp(-R2.*y) + R2*C4.*exp(R2.*y) ...
                + (u0*gamma_two.*v.*B0.*(-p.*exp(-p.*(y+d1))))./(k2.^2))...
                .*exp(1i.*p.*z + omega.*t);
end
