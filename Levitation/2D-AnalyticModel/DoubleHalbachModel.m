function [Force_y, Force_z, n] = DoubleHalbachModel(v,geometry,tau_factor,...
...
%   geometry: 'Single' vs. 'Double' sided arrays
...
if(strcmp(profile,'Brakes'))
    Bym = @(y) B0.*(exp(-p*(y+d1)) + exp(-p*(-y+d2+1)));
end
...
if(strcmp(profile,'Brakes'))
   Bz = @(y,z) Bsz(y,z) - (-R2*C3*exp(-R2.*y) + R2*C4.*exp(R2.*y) ...
        +(f*(-p*exp(-p*(y+d1))-p*exp(-p*(-y+d2+l))))).*exp(1i.*p.*z + omega.*t); 
%      Bz = @(y,z) Bsz(y,z) - (-R2*C3*exp(-R2.*y) + R2*C4.*exp(R2.*y) ...
%           +(f*(-p*exp(-p*(y+d1))))).*exp(1i.*p.*z + omega.*t); 
end
